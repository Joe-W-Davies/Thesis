...
The intial CMS tracker design comprises an inner pixel detector and a strip detector, covering the pseudorapdity range $|\eta|\,<\,2.5$.
...
The CMS ECAL is a homogeneous detector which uses lead tungstate ($\textrm{PbWO}_4$) crystals as the active material.
A total of 61,200 crystals cover the barrel area (EB), with 7324 in each of the endcaps (EE).
An additional preshower detector is present in front of the endcaps, in order to remove $\pi_0$ rejection.
The total coverage in pseudirapdiity extends to $|\eta|=3$; however full energy resolution is maintained only to ~$|\eta|=2.5$.
