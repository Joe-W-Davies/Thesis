...
% \begin{equation}\label{eq:category_likelihood}
%     L_k({\rm{data}}\,|\,\mu^{i,\gamma\gamma},m_{\rm{H}},\vec{\theta}_s,\vec{\theta}_b) = \prod^{N_{\rm{bins}}}_{X} {\rm{Poisson}} \Big( N_{k,X}^{\rm{data}} \, \Big| \, \Big[ \sum_{i} S_{k,X}^{i,\gamma\gamma}(\mu^{i,\gamma\gamma},m_{\rm{H}},\vec{\theta}_s)\Big] + B_{k,X}(\vec{\theta}_b) \Big),        
% \end{equation}
...
%In previous CMS \Hgg analyses, each of these parameters, in addition to $f_{{\rm{RV}}}$, were represented by a linear function of $m_{\rm{H}}$, where the exact form of these functions was derived by simultaneously fitting simulated events corresponding to three values of $m_{\rm{H}}$: 120, 125, and 130~GeV. Using this approach, the final signal model shape was defined as a continuous parametric function of $m_{\rm{H}}$. Ultimately, this choice was made since the value of $m_{\rm{H}}$ was profiled when extracting the final results, reflecting the lack of knowledge with respect to $m_{\rm{H}}$ at the time. 
%In this analysis the value of $m_{\rm{H}}$ is fixed to 125.38~GeV~\cite{Sirunyan:2020xwk}. Instead, shape parameters are derived using simulated events with $m_{\rm{H}}$~=~125.0~GeV only, where the width and relative contribution of each Gaussian function are now defined as constants with respect to $m_{\rm{H}}$, and the mean values are shifted up by 380~MeV. 
...
\begin{table}
    \caption[Function families considered in the discrete profiling method]{Different function families considered for modelling the background with the discrete profiling method. Each function is shown for order, $N$.}
...
% The combined effect of these uncertainties on the inclusive Higgs boson signal-strength measurement is between 3 and 4\%.
