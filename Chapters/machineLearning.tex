...
Suppose that a model is to be built on a dataset $\{\vec{x}_{i},y_{i}\}$, with $i=1,2, ..., n,$ indexing the number of data points to learn from. The process to construct a gradient boosted decision tree, $F_{m}$, where $m=0,1, ..., M,$ labels the iteration of the boosting algorithm iteration, is summarised as follows: %Note that the F's are not residuals, but the actual values of the variable being predicted_
...
\begin{itemize}
    ...
    %why tanh not sigmoid? Its zero centred and so long sum operations distributions the gradients better i.e. equal negative and equal positive in general. In the example of the sentence prediction, we would want to add the gender of the new subject to the cell state, and replace the old one to be forgotten.
...
%So in our case, our sequence is the three jet vectors, and each cell takes one of the individual vectors of jet values i.e. t goes from 1 to 3. In the HP opt we can have multiple LSTMs, but I think they are self contained i.e. the initial vectors are transformed from LSTM BLOCK 1, which itself has many LSTM cells, and the transformed sequence is then fed into LSTM BLOCK 2 etc.
