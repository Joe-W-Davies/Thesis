...
Each bin is exclusive; events passing the VBF-like selection enter the VBF-like region
...
except for the additional requirement that the leading electron has $\pt > \SI{40}{GeV}$.
...
and then integrating the number of events between $\SI{125}{GeV} - \sigma_{eff} < \mH < \SI{125}{GeV} + \sigma_{eff}$.
...
the categories for each of these scenarios.
...
does not significantly improve the performance of the dijet BDT;
...
\begin{figure}
  ...
  \caption[The data-driven method for dijet BDT training.]
  {
    ...
    The fake-factor for the leading photon is denoted by $f(\gamma_1)$ 
    and the fake-factor for the subleading photon by $f(\gamma_2)$.
...
but with the requirement that the diphoton mass does not lie in the region $\SI{115}{GeV} < \mgg < \SI{135}{GeV}$.
These events cannot be used for training the dijet BDT since their reuse in the final fits 
could then induce bias; 
however they are useful for validating the method, 
since their kinematic properties should be essentially identical to the data-driven output.
